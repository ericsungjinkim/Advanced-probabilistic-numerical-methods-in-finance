...
class HJBLQ(Equation):
    ...
    '''
    def __init__(self, eqn_config,lamdb = 1.0):
...
class NonsharedModel(torch.nn.Module):
    def __init__(self, config, bsde):
        ...
        z_init = torch.tensor(np.random.uniform(low=-.1, high=.1,size=[1, self.eqn_config["dim"]]),
                                   dtype=dtype,requires_grad=True)
...
HJB_bsde = HJBLQ(HJB_config["eqn_config"],1.0)
