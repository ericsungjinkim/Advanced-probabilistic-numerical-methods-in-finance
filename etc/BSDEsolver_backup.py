    ...
    def train(self):
        ...
        for step in range(self.net_config["num_iterations"]+1):
            ...
            if step % 100 == 0:
                ...
                print("step ",step," err_rate : " ,err_rate)

        return np.array(err_rates), np.array(training_history)
...
class NonsharedModel(torch.nn.Module):
    def __init__(self, config, bsde):
        ...
        self.z_init = torch.tensor(np.random.uniform(low=-.1, high=.1,
                                                     size=[1, self.eqn_config["dim"]]),dtype=dtype)
    ...
    def forward(self, inputs, training):
        ...
        for t in range(0, self.bsde.num_time_interval - 1):
            ...
            ) + torch.mm(z, dw[:, :, t].t()).sum(1)).reshape([batch_size,1])
...
Default_Risk_bsde_solver.train()